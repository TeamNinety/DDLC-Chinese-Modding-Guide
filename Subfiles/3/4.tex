...
\begin{ExtraKnowledge}
    在define语句中同样可以指定优先级，只需要在define关键词后添加优先级即可，如：\verb|define 3 demo = True|。
\end{ExtraKnowledge}
...
一般来说，在使用DDLC中文Mod模板制作且没有对模板进行设置的游戏，在Ren'Py标准位置里会有一个名为DDLCModTemplateZh的文件夹，里面通常有一个名为persistent的文件，那就是存储持久化数据的存档文件。
