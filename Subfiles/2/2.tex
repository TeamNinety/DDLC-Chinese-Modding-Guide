...
在\ref{subsec:2.2.1}课中，我们学习了如何在视觉小说中编写对话。但现在有一个很大的问题⸺没有图像。在视觉小说中，视觉便是重点。在本课中，我们将学习如何在视觉小说中显示图像。
...
模板中已经将所有可能的身体姿势为我们组合好了。除优里外的所有角色都有五个姿势（1-4为正常站位，5为特殊站位），而优里只有4个姿势（1-3为正常站位，4为特殊站位）。
...
\begin{center}
    ...
    \tabletail{\hline}
    \tablelasttail{\hline}
    \tablecaption{常用的show语句附加参数}
    \begin{supertabular}{|c|c|}
        \hline
        at <transform> & 将一个动画(transform)套用到立绘上\\
        \hline
        with <transition> & 显示图像时使用指定的转场动画(transition)\\
        \hline
        zorder <数字> & 控制图像在Z轴上的位置\\
        \hline
    \end{supertabular}
\end{center}
...
变换（transform）与转场（transition），就如名字一样，定义了一个图像在屏幕上的变化内容与方式，即动画、运动方式、位置、透明度等。变换与转场使用相同的方式定义——动画与变换语言（ATL）。当一个变换作用于角色立绘时，应使用at从句。一个例子：t41。在这个变换中，t表示在这个位置上的角色静止站立在原地；4表示一共有4个站位，会将屏幕等分成4份；1表示从左往右该变换是所有站位中的第1个。
...
套用转场动画时，应使用with从句。游戏中实现定义好的转场动画有：
...
\begin{ExtraKnowledge}
    hide语句同样可以使用onlayer从句，用于隐藏对应图层上的图像。但在游戏中很少会涉及到对图层的操作，故本书不会对onlayer从句展开讲解。有兴趣者课前往 \url{https://doc.renpy.cn/zh-CN/displaying_images.html} 了解详细内容。
\end{ExtraKnowledge}
